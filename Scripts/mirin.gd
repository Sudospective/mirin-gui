...
func _on_insert():
	...
	if cur_panel != set_panel:
		...
		if cur_panel.get_node("ModFlip").pressed:
			m_ease = 'flip(' + m_ease + ')'
...
func insert_mod(type: String, start: String, length: String, easefunc: String, percent: String, mod: String):
	...
	mods.append('\t' + modline)
	emit_signal("add_to_modlist", type, modline)
...
func save_to_file(file):
	...
	split_file.insert(0, '<!-- AUTO-GENERATED BY MIRIN TEMPLATE GUI EDITOR -->')
